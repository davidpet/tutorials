...
#    If you want to have the last rule in the chain have the same name as the name passed in,
...
# 5. Use **kwargs to forward extra args from a macro call to an underlying rule.
# 6. Rely on 'bazel run' commands for things with side effects.
#    eg. have the user bazel run the final target (with certain suffix) to do the chain.
#        (such as publishing a docker file or whatever)
