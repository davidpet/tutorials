...
# This rule is called by subfolder/BUILD with srcs = ["file1.txt", "file2.txt"] and
# deps = [":using_touch"] and outs = ["path_handling_out.txt"].
# srcs and deps aren't handled differently by bazel but rather by the code below.
# so in terms of short path and root usage, you can assume the same targets would
# cause the same behavior either way.
# NOTE: if you try to declare_file on a path outside the package (eg. from subfolder into
#       parent folder) it won't work.  I think this is true in genrule as well.
#       This is a challenge for things like compiling targets that are transitive
#       dependencies that don't come from your package.  That is probably why the
#       officialy python_proto_library rule has the weird repeat of the package path.
def _path_handling_impl(ctx):
    srcs = ctx.files.srcs
    deps = ctx.files.deps
    outputs = []

    # bazel-out/darwin_arm64-fastbuild/bin
    # summary: base of the workspace level in the output within the sandbox
    print("bin path: {}".format(ctx.bin_dir.path))

    for src in srcs:
        # subfolder/file1.txt, subfolder/file2.txt
        # summary: workspace-relative paths of the files
        print("src path: {}".format(src.path))
        # same as above
        print("src short path: {}".format(src.short_path))
        # empty
        print("root: {}".format(src.root.path))
    for dep in deps:
        # bazel-out/darwin_arm64-fastbuild/bin/using_touch_out.txt
        # summary: workspace-relative paths of the files
        #          in this case because generated by another rule, they go into bazel-out subfolder
        print("dep path: {}".format(dep.path))
        # using_touch_out.txt
        # summary: workspace-relative path with all the bazel-out stuff stripped away
        print("dep short path: {}".format(dep.short_path))
        # bazel-out/darwin_arm64-fastbuild/bin
        # summary: the part within worksapce sandbox that short_path is relative to
        #          in this case, it's ctx.bin_dir.path
        print("root: {}".format(dep.root.path))
    
    for src in srcs:
        # if were to try it in a non-package path (eg. the parent folder),
        # this would fail() the whole build.
        rel_src = paths.relativize(src.path, ctx.label.package)
        # file1.txt, file2.txt
        print("relative src: {}".format(rel_src))
        output = ctx.actions.declare_file(rel_src + "_out.txt")
        # bazel-out/darwin_arm64-fastbuild/bin/subfolder/file1.txt_out.txt, bazel-out/darwin_arm64-fastbuild/bin/subfolder/file2.txt_out.txt
        print("declare_file path: {}".format(output.path))
    ...
    for i in range(len(srcs)):
        ...
        # the ls command shows the following present:
        #   file1.txt or file2.txt in subfolder of sandbox (only 1 at a time due to the loop)
        #   using_touch_out.txt in ctx.bin_dir of sandbox
        ctx.actions.run_shell(
            inputs = deps + [src],
            outputs = [output],
            use_default_shell_env = True,
            command = """
                ls -R -1 > "{0}"
                echo fake > "{0}_double_out.txt"
            """.format(output.path),
        )
    
    # the ls command shows only using_touch_out.txt and the outs from the commands above present
    #   all are relative paths to workspace and are located within ctx.bin_dir
    ...
    # Only the files mentioned below get copied from ctx.bin_dir into bazel-bin root at the end.
    # If you comment out either part, those files don't show up in bazel-bin.
    # Even though I tried to tell it to include file1.txt in bazel-bin by appending that, it
    # didn't work.  However, a downstream rule should be able to see it as it is without having
    # to look in bazel-bin.
    # The paths below are within ctx.bin_dir, and downstream rules that rely on those paths
    # get the bin_dir versions.
    # Presumably, that means the build system records all the paths seen by DefaultInfo providers
    # and remembers them for the end of the build step.
    # After building, the ctx.bin_dir version is gone and just the bazel-bin version remains, so
    # presumably that's what the 'bazel run' command would be running.
    return [DefaultInfo(files = depset(outputs + ctx.outputs.outs + [srcs[0]]))]

path_handling = rule(
    implementation = _path_handling_impl,
    attrs = {
        "srcs": attr.label_list(allow_files = True),
